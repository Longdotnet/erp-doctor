...
The Windows and Linux archives are self-contained .NET 8 builds. PostgreSQL, Docker, and Linux/Nginx are shipped as separate provider bundles because plugins remain an explicit trust/install boundary.
...
- publishes PostgreSQL, Docker, and Linux/Nginx provider bundles,
- runs the standalone Linux binary,
- verifies that standalone binary can discover all bundled provider DLLs with expected check counts,
...
The smoke test does not execute PostgreSQL/Docker/Nginx diagnostics against a live service. Live-service permissions and availability remain deployment-specific runtime concerns.
...
4. PostgreSQL, Docker, and Nginx provider archives are present.
5. The standalone Linux binary discovers all three providers with expected check counts.
